...
def add_previus(tag_info):
        color = 'purple'
        intersection = tag_info
        if intersection.geom_type == 'MultiPolygon':
            ...
            plt.fill(x, y, alpha=0.2, color=color)
            plt.plot(x, y, '--', color=color, alpha=0.5, label=f'Tag Area')
...
def process_tag_data(excel_file):
    ...
    for sheet_name in xl.sheet_names:
        ...
        if common_intersection is not None and not common_intersection.is_empty:
            ...
            common_intersection2 = common_intersection
            ...
            all_tags_data_vs[sheet_name] = {
                'tag_x': tag_x,
                'tag_y': tag_y,
                'centroids': centroids,
                'intersection_polygon': common_intersection2
            }    

            all_tags_data.append(tag_data)
        #print (all_tags_data)
        plt.show()
    create_all_tags_plot(all_tags_data_vs)
    time.sleep(2)
    plt.show()
