...
def process_tag_data(excel_file):
    ...
    for sheet_name in xl.sheet_names:
        ...
        ##create_combined_plot(all_data, tag_x, tag_y, sheet_name) ##Niet nodig voor berekeningen alleen plot
        ##create_intersection_plot(all_data, tag_x, tag_y, sheet_name)##Niet nodig voor berekeningen alleen plot
        shapely_polygons = []
        angles = np.linspace(-ALPHA, ALPHA, int(ALPHA * 2 + 1))
...
if __name__ == "__main__":
    ...
    ## Start process
    try:
        results = process_tag_data(latest_file)
        print(results)
