...
\textbf{Note: this will start the LiDAR without blind spots. If you want to start with blind spots, replace: \\ \highLight[LightGrey]{''\texttt{cartographer.launch.py}''} with: \highLight[LightGrey]{''\texttt{cartographer\_w\_blindspots.launch.py}''}.}\\
...
Figure \ref{fig:RVIZ_Cartographer} shows the generated map in RVIZ.
...
sometimes saving the map gives the following error message: ''\textcolor{Red}{\texttt{Failed to spin map subscription}}''. Figure \ref{fig:map_saver_fail} shows the failed output. This error has two possible solutions.

\begin{enumerate}
	\item The ''\texttt{-f}'' tag doesn't always work. Try a different directory, name or remove the tag all together.
	\begin{itemize}
		\item If no tag is given the program will save with a random name in the current directory.
	\end{itemize}
	\item Sometimes there is no reason. If you retry it again without doing anything different it just works.
\end{enumerate}
