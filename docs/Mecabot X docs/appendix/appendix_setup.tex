...
This can be done in three different ways. If you're using Windows you can setup WSL (\ref{sec:appendix_wsl_setup}). If you're dual booting or only using Linux follow appendix \ref{sec:appendix_linux_setup}.

\textbf{NOTE: WSL and VM can't directly communicate with the} \texttt{ROS\_DOMAIN\_ID} \textbf{This means you can't (easily) visualise what the Mecabot X is doing using these options.}
...
\paragraph{Install ROS2 Humble}

\subparagraph{Set locale}
The locale needs to be \texttt{UTF-8} you can check using the following command:
...
Set locale to \texttt{en\_US.UTF-8}:
...
The following adds the ROS2 Humble packages to the repository
...
\subparagraph{Install ROS 2 packages}

First update the repository and upgrade to the newest version
...
After updating you can install the ROS Humble desktop package using the following command.
...
Optionally you can install the ROS dev tools. These are needed to re-compile the existing binary's.
...
Now ROS should be installed on the PC and you can run the ROS commands in this manual.
