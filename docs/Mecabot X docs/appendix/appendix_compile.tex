...
\subsubsection{Create working directory} %INITIAL SETUP

The source code can be found on the Re-manufacturing lab GitHub repository. It can be found in: ''\texttt{TagMappingRobot/src/wheeltec\_ros2}''. The source code is based on the original code made by Roboworks\cite{mecabot_x_git}.
...
\textbf{NOTE: If compiling on PC ''astra\_camera'' will always fail to compile. By adding a file named: ''\texttt{COLCON\_IGNORE}'' (Needs to be capitalized) in the folder: ''src/transbot/core/ros2\_astra\_camera'' it will not compile the camera function and it will continue.} \\
...
\subsubsection{Recompile the source}

To recompile you can rerun the colcon build command. Or for a clean rebuild you'll need to delete the ''install'' and ''build'' folder.
